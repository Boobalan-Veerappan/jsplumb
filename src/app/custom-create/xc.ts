// import { Graph, Layout, Edge } from '@swimlane/ngx-graph/lib/ngx-graph.module';
import { Edge, Graph } from '@swimlane/ngx-graph/lib/models';
...
export class DagreNodesOnlyLayout implements Layout {
  ...
  public run(graph: Graph): Graph {
    ...
    console.log(" @@@@@@  ",graph.nodes[0].position?.x);
    

    this.createDagreGraph(graph);
    dagre.layout(this.dagreGraph);
console.log(" degree Graph ",this.dagreGraph._nodes);


    graph.edgeLabels = this.dagreGraph._edgeLabels;

    for (const dagreNodeId in this.dagreGraph._nodes) {
        console.log(" ### ",dagreNodeId);
  ...
  public updateEdge(graph: Graph, edge: Edge): Graph {
    ...
    const sc = sourceNode?.position!;
    const tp = targetNode?.position!;
    const sd = sourceNode?.dimension!;
    const td = targetNode?.dimension!;
    console.log(" %%% ",rankAxis);
    
    const dir = sc[rankAxis] <= tp[rankAxis] ? -1 : 1;
    const startingPoint = {
      [orderAxis]: sc[orderAxis],
      [rankAxis]: sc[rankAxis] - dir * (sd[rankDimension] / 2)
    };
    const endingPoint = {
      [orderAxis]: tp[orderAxis],
      [rankAxis]: tp[rankAxis] + dir * (td[rankDimension] / 2)
    };
   ...
   console.log(" edgeLabelId ",edgeLabelId);
  ...
  public createDagreGraph(graph: Graph): any {
    console.log(" defaultSetting ",this.defaultSettings);
    
    const settings = Object.assign({}, this.defaultSettings, this.settings);
    console.log(" compound ",settings.compound);
    console.log(" setting ",settings.multigraph);
